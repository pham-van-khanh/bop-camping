...
class VoucherService
{
     ...
     */
    public function quote(int $base, Collection $vouchers, PromotionSetting $settings): array
    {
        $base = max(0, $base);

        $lines = $vouchers->map(function (Voucher $v) use ($base) {
            $raw = $v->type === 'percent'
                ? (int) floor($base * (float) $v->value / 100)
                : (int) round((float) $v->value);

            return ['code' => $v->code, 'raw' => max(0, min($raw, $base))];
        })->values();
    ...
    public function apply(Order $order, array $codes, ?PromotionSetting $settings = null): array
    {
        ...
        return DB::transaction(function () use ($order, $codes, $settings, $base) {
            ...
            $quote = $this->quote($base, $vouchers, $settings);
