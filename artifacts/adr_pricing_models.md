...
- **Ngày:** 2026-07-24
- **Beads:** sẽ tạo bead riêng (Phase 3 — sau Phase 1/2 của Turnaround)
- **Liên quan:** `app/Services/AvailabilityService.php`, `app/Services/PricingService.php` (mới / nơi tính tiền), `products`, `product_service_location`, `order_items`, `site_settings`.
...
- **Gắn cờ "nửa ngày" ở cấp đơn** (`is_half_day`). Cờ này để admin **thấy ngay đơn nào sẽ trả vào trưa** → là tín hiệu cân nhắc quay vòng (mục 6).
...
```
// migration: add_half_day_flag_to_order_items  (hoặc orders, tuỳ mô hình đơn hiện tại)
...
## 5. Tính tiền (`PricingService`)

- **Đơn cùng ngày, trả sớm (`is_half_day`):** `giá_ngày × (1 − early_return_discount_pct/100)`.
- **Đơn cùng ngày, cả ngày:** `giá_ngày`.
- **Đơn nhiều ngày:** `giá_ngày × N` (N theo quy ước mục 2). Không áp ưu đãi trả sớm.
...
## 9. Kế hoạch triển khai (Phase 3 — bead riêng)

1. Migration: `early_return_discount_pct` (+ tùy chọn `accepting_orders`) vào `products`; `is_half_day` vào `order_items`.
2. `PricingService`: công thức theo mục 5.
3. Checkout: đơn cùng ngày hiện lựa chọn "trả sớm trong ngày (−%)"; set `is_half_day`. Đơn nhiều ngày không hiện bậc này.
