...
class AdminServiceLocationTest extends TestCase
{
    ...
    /** @test */
    public function guest_cannot_access(): void
    {
        $this->get(route('admin.service-locations'))->assertRedirect(route('admin.login'));
    }
    ...
    /** @test */
    public function index_lists_locations_with_spot_count(): void
    {
        $loc = ServiceLocation::create(['name' => 'Hà Nội', 'status' => 'open']);
        ...
        $this->actingAs($this->admin())->get(route('admin.service-locations'))
            ->assertInertia(fn ($page) => $page
                ->component('Admin/ServiceLocations')
                ->has('locations', 1)
                ->where('locations.0.spots_count', 2));
    }
}
