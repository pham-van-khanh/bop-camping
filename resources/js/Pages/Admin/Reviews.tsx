...
const Stars = ({ n }: { n: number }) => (
    <span className="font-mono text-[13px]" style={{ color: '#C97B36' }}>{'★'.repeat(n)}<span className="text-[#d8dcc8]">{'★'.repeat(5 - n)}</span></span>
);
...
export default function AdminReviews() {
    ...
    const go = (patch: Record<string, string>) =>
        router.get(route('admin.reviews'), { ...filters, ...patch }, { preserveScroll: true, replace: true });

    const decide = (id: number, status: 'approved' | 'rejected', admin_note = '') =>
        router.patch(route('admin.reviews.update', id), { status, admin_note }, { preserveScroll: true, onSuccess: () => { setRejectId(null); setNote(''); } });

    return (
        <>
            <Head title="Admin · Đánh giá" />
            <div className="mx-auto max-w-[920px]">
                <h1 className="mb-1 text-[22px] font-extrabold text-ink">Duyệt đánh giá</h1>
                <p className="mb-4 text-[14px] text-moss">Chỉ đánh giá đã duyệt mới hiển thị công khai.</p>
                ...
                <div className="mb-4 flex flex-wrap items-center gap-3">
                    <div className="flex gap-1 rounded-[12px] border border-cardBorder bg-white p-1">
                        {STATUS_TABS.map((t) => (
                            <button key={t.key} onClick={() => go({ status: t.key })}
                                ...
                                {t.label}{t.key === 'pending' && counts.pending > 0 ? ` (${counts.pending})` : ''}
                            </button>
                        ))}
                    </div>
                    <div className="w-[180px]">
                        <SelectInput value={filters.category} onChange={(v) => go({ category: v })} ariaLabel="Loại đánh giá"
                            options={[{ value: 'all', label: 'Tất cả loại' }, { value: 'product', label: 'Sản phẩm' }, { value: 'system', label: 'Hệ thống' }]} />
                    </div>
                </div>

                {reviews.data.length === 0 ? (
                    <div className="rounded-card border border-cardBorder bg-card py-12 text-center text-[14px] text-moss">Không có đánh giá nào.</div>
                ) : (
                    <div className="flex flex-col gap-3">
                        {reviews.data.map((r) => (
                            <div key={r.id} className="rounded-card border border-cardBorder bg-card p-4">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        <div className="flex flex-wrap items-center gap-2">
                                            <span className="font-bold text-ink">{r.reviewer_name}</span>
                                            <Stars n={r.rating} />
                                            <span className="rounded-pill px-2 py-0.5 text-[11px] font-semibold" style={{ color: STATUS_PILL[r.status].c, background: STATUS_PILL[r.status].b }}>{STATUS_PILL[r.status].t}</span>
                                            <span className="rounded-pill bg-[#eef2e3] px-2 py-0.5 font-mono text-[11px] text-moss">{r.category === 'system' ? 'Hệ thống' : 'Sản phẩm'}</span>
                                        </div>
                                        <div className="mt-0.5 font-mono text-[12px] text-moss">
                                            {r.product ? r.product.name + ' · ' : ''}{r.created_at}
                                        </div>
                                    </div>
                                </div>

                                {r.content && <p className="mt-2 text-[14px] leading-[1.55] text-ink">{r.content}</p>}
                                ...
                                {r.status === 'rejected' && r.admin_note && (
                                    <p className="mt-2 text-[12.5px] text-[#b3493a]">Lý do từ chối: {r.admin_note}</p>
                                )}
                                ...
                                {rejectId === r.id ? (
                                    <div className="mt-3 flex flex-wrap items-center gap-2">
                                        ...
                                        <button onClick={() => decide(r.id, 'rejected', note)} className="h-10 rounded-control bg-[#b3493a] px-4 text-[13px] font-bold text-white">Xác nhận từ chối</button>
                                        <button onClick={() => { setRejectId(null); setNote(''); }} className="h-10 rounded-control border border-cardBorder px-4 text-[13px] font-semibold text-pine">Huỷ</button>
                                    </div>
                                ) : (
                                    <div className="mt-3 flex flex-wrap gap-2">
                                        {r.status !== 'approved' && (
                                            <button onClick={() => decide(r.id, 'approved')} className="h-9 rounded-control bg-grass px-4 text-[13px] font-bold text-white transition hover:bg-pine">✓ Duyệt</button>
                                        )}
                                        {r.status !== 'rejected' && (
                                            <button onClick={() => { setRejectId(r.id); setNote(''); }} className="h-9 rounded-control border border-[#f6ddd6] px-4 text-[13px] font-semibold text-[#b3493a] transition hover:bg-[#f6ddd6]">Từ chối</button>
                                        )}
                                    </div>
                                )}
                            </div>
                ...
                {reviews.last_page > 1 && (
                    <div className="mt-4 flex items-center justify-between text-[12.5px] text-moss">
                        <span className="font-mono">{reviews.from}–{reviews.to} / {reviews.total}</span>
                        <div className="flex gap-2">
                            <button disabled={reviews.current_page <= 1} onClick={() => go({ page: String(reviews.current_page - 1) })} className="rounded-[8px] border border-cardBorder px-3 py-1.5 font-semibold text-pine disabled:opacity-40">Trước</button>
                            <button disabled={reviews.current_page >= reviews.last_page} onClick={() => go({ page: String(reviews.current_page + 1) })} className="rounded-[8px] border border-cardBorder px-3 py-1.5 font-semibold text-pine disabled:opacity-40">Sau</button>
                        </div>
                    </div>
                )}
            </div>
