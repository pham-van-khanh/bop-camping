...
const STATUS_LABEL: Record<string, string> = { pending: 'Chờ', converted: 'Thành công', rejected: 'Từ chối', expired: 'Hết hạn' };

export default function AdminReferrals() {
    const { referrals, filters } = usePage<Props>().props;

    const filter = (status: string) =>
        router.get(route('admin.referrals'), { status }, { preserveState: true, replace: true });
                ...
                <table className="w-full text-[13px]">
                    <thead>
                        <tr className="border-b border-cardBorder text-left text-moss" style={{ background: '#f8faf4' }}>
                            <th className="px-3 py-2.5 font-semibold">Người giới thiệu</th>
                            <th className="px-3 py-2.5 font-semibold">Người được mời</th>
                            <th className="px-3 py-2.5 font-semibold">Mã</th>
                            <th className="px-3 py-2.5 font-semibold">Đơn đầu</th>
                            <th className="px-3 py-2.5 font-semibold">Trạng thái</th>
                            <th className="px-3 py-2.5 font-semibold">Chuyển đổi</th>
                        </tr>
                    </thead>
                    <tbody>
                        {referrals.data.length === 0 && (
                            <tr><td colSpan={6} className="px-3 py-6 text-center text-moss">Chưa có lượt giới thiệu nào.</td></tr>
                        )}
                        {referrals.data.map((r) => (
                            <tr key={r.id} className="border-t border-[#eef2e3]">
                                <td className="px-3 py-2.5 text-ink">{party(r.referrer)}</td>
                                <td className="px-3 py-2.5 text-ink">{party(r.referee)}</td>
                                <td className="px-3 py-2.5 font-mono font-bold text-pine">{r.code_used}</td>
                                <td className="px-3 py-2.5 font-mono text-grass">{r.first_order_code ?? '—'}</td>
                                <td className="px-3 py-2.5">
                                    <span className="rounded-pill px-2.5 py-1 text-[11px] font-bold" style={STATUS_STYLE[r.status]}>{STATUS_LABEL[r.status]}</span>
                                </td>
                                <td className="px-3 py-2.5 text-moss">{r.converted_at ?? r.created_at}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
