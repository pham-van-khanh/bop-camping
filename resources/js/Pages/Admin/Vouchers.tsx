...
export default function AdminVouchers() {
    ...
    const submitCreate = (e: React.FormEvent) => {
        e.preventDefault();
        create.post(route('admin.vouchers.store'), {
            preserveScroll: true,
    ...
    const revoke = (id: number) =>
        router.patch(
            route('admin.vouchers.revoke', id),
            {},
            { preserveScroll: true },
        );
        ...
        <>
            ...
            <div className="p-6">
                ...
                <form
                    ...
                    className="mb-5 flex flex-wrap items-end gap-3 rounded-card border border-cardBorder bg-card p-4"
                >
                    <Field label="SĐT khách">
                        <input
                            value={create.data.phone}
                            onChange={(e) =>
                                create.setData('phone', e.target.value)
                            }
                            placeholder="09..."
                            className={inputCls}
                        />
                    ...
                    <Field label="Giá trị">
                        <input
                            type="number"
                            step="any"
                            value={create.data.value}
                            onChange={(e) =>
                                create.setData('value', e.target.value)
                            }
                            className={inputCls}
                        />
                        {create.errors.value && (
                    ...
                    <Field label="HSD (ngày)">
                        <input
                            type="number"
                            value={create.data.validity_days}
                            onChange={(e) =>
                                create.setData('validity_days', e.target.value)
                            }
                    ...
                    <button
                        ...
                        className="h-11 rounded-control bg-grass px-5 text-[14px] font-bold text-white disabled:opacity-60"
                    >
                        Tạo voucher
                    </button>
                </form>
                ...
                <div className="mb-3 flex flex-wrap items-center gap-2">
                    ...
                    <SelectInput
                        ...
                        className="w-[190px]"
                        options={Object.entries(VOUCHER_SOURCE_LABEL).map(
                            ([value, label]) => ({ value, label }),
                        )}
                    />
                </div>
                                ...
                                >
                                    ...
                                    <td className="px-3 py-2.5 font-semibold text-grass">
                                        ...
                                        {v.applicable_to_combos && (
                                            <span
                                                className="ml-1.5 rounded-pill bg-[#e7eed5] px-1.5 py-0.5 font-mono text-[10px] font-bold text-[#3a5a1f]"
                                                title="Voucher này giảm được cả phần combo"
                                            >
                                                +COMBO
                                            </span>
                                        )}
                                    ...
                                    <td className="px-3 py-2.5 text-right">
                                        {v.status === 'active' && (
                                            <button
                                                onClick={() => revoke(v.id)}
                                                className="text-[12px] font-semibold text-[#b3493a] hover:underline"
                                            >
                                                Thu hồi
                                            </button>
                                        )}
...
const inputCls =
    'h-11 rounded-[10px] border border-cardBorder bg-white px-3 text-[14px] text-ink outline-none focus:border-grass';
        ...
        <div className="mt-4 flex items-center justify-center gap-3 text-[13px]">
            <button
                ...
                onClick={() =>
                    ...
                    router.get(paginator.prev_page_url)
                }
                className="rounded-control border border-cardBorder px-3 py-1.5 font-semibold text-pine disabled:opacity-40"
            >
                ← Trước
            </button>
            ...
            <button
                ...
                onClick={() =>
                    ...
                    router.get(paginator.next_page_url)
                }
                className="rounded-control border border-cardBorder px-3 py-1.5 font-semibold text-pine disabled:opacity-40"
            >
                Sau →
