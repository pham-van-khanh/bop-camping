...
export default function AdminFeedbacks({
    ...
    const applyFilter = (status: string) =>
        router.get(route('admin.feedbacks'), status ? { status } : {}, { preserveState: true, replace: true });
        ...
        <div className="p-6">
            ...
            <div className="mb-5">
                ...
                <p className="mt-1 text-[13px] text-moss">
                    Góp ý gửi từ widget trên website. Phản hồi qua email sẽ gửi bằng địa chỉ cấu hình trong hệ thống.
                </p>
            </div>
                ...
                {FILTERS.map((f) => (
                    <button
                        ...
                        className={`rounded-pill px-4 py-2 text-[12.5px] font-semibold transition ${
                            (filters.status || '') === f.value ? 'bg-grass text-white' : 'border border-cardBorder bg-white text-pine hover:border-grass'
                        }`}
                    >
                        {f.label}
                    </button>
            ...
            {feedbacks.data.length === 0 ? (
                ...
                <div className="space-y-3">
                    {feedbacks.data.map((f) => (
                        <FeedbackCard key={f.id} f={f} open={openId === f.id} onToggle={() => setOpenId(openId === f.id ? null : f.id)} />
                    ))}
                </div>
            )}
            ...
            {feedbacks.last_page > 1 && (
                <div className="mt-4 flex items-center justify-between text-[12.5px] text-moss">
                    <span className="font-mono">{feedbacks.from}–{feedbacks.to} / {feedbacks.total}</span>
...
function FeedbackCard({ f, open, onToggle }: { f: FeedbackRow; open: boolean; onToggle: () => void }) {
    const form = useForm({ reply_content: f.reply_content ?? '' });

    const send = (e: React.FormEvent) => {
        e.preventDefault();
        form.patch(route('admin.feedbacks.reply', f.id), { preserveScroll: true });
    };

    return (
        <div className="overflow-hidden rounded-[14px] border border-cardBorder bg-white">
            <button onClick={onToggle} className="flex w-full flex-wrap items-center gap-3 px-4 py-3.5 text-left transition hover:bg-[#fafcf7]">
                <span
                    className={`rounded-pill px-2.5 py-1 text-[11.5px] font-bold ${
                        f.status === 'new' ? 'bg-[#f7e7da] text-[#8a5a1f]' : 'bg-[#dcebc4] text-[#3a5a1f]'
                    }`}
                >
                    {f.status === 'new' ? 'Chưa phản hồi' : 'Đã phản hồi'}
                </span>
                <span className="min-w-[120px] text-[14px] font-bold text-pine">{f.name}</span>
                <span className="min-w-0 flex-1 truncate text-[13px] text-moss">{f.content}</span>
                <span className="font-mono text-[11.5px] text-moss">{f.created_at}</span>
            </button>

            {open && (
                <div className="border-t border-[#f1f4ea] px-4 py-4" style={{ background: '#fafcf7' }}>
                    <div className="mb-3 flex flex-wrap gap-x-5 gap-y-1 text-[13px] text-moss">
                        {f.phone && (
                            <span>📞 <a href={`tel:${f.phone}`} className="font-mono font-bold text-grass hover:underline">{f.phone}</a></span>
                        )}
                        {f.email && <span>✉️ <span className="font-semibold text-pine">{f.email}</span></span>}
                        {f.replied_at && <span>Đã phản hồi lúc {f.replied_at}</span>}
                    </div>
                    ...
                    <form onSubmit={send}>
                        <label className="mb-1.5 block text-[13px] font-semibold text-pine">
                            ...
                            {f.email ? (
                                <span className="ml-1 font-normal text-moss">
                                    — email gửi tới khách tự kèm lời chào theo tên + cảm ơn + chữ ký shop, bạn chỉ soạn phần trả lời
                                </span>
                            ) : (
                                <span className="ml-1 font-normal text-campfire">
                                    — khách không để email: liên hệ qua SĐT/Zalo rồi lưu ghi chú tại đây
                                </span>
                            )}
                        </label>
                        <textarea
                        ...
                        />
                        {form.errors.reply_content && <p className="mt-1 text-[12px] text-[#b3493a]">{form.errors.reply_content}</p>}
                        <div className="mt-2.5 flex justify-end">
                            ...
                            >
                                {form.processing ? 'Đang gửi…' : f.email ? (f.status === 'replied' ? 'Gửi lại email phản hồi' : 'Gửi email phản hồi') : 'Lưu & đánh dấu đã phản hồi'}
                            </button>
                        </div>
                    </form>
                </div>
