...
export default function AdminBanners({
    ...
    const blank = (): FormData => ({
        placement: placementKeys[0] ?? 'hero',
        image: null,
        title: '',
        subtitle: '',
        link_url: '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (modalMode === 'create')
            form.post(route('admin.banners.store'), opts);
        else if (editing) {
    ...
    const toggleActive = (b: Banner) => {
        ...
        router.post(route('admin.banners.update', b.id), fd, {
            forceFormData: true,
            preserveScroll: true,
        });
    };
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-0.5 text-[13px] text-moss">
                            Ảnh slideshow đầu trang (hero) và dải khuyến mãi
                            (promo)
                        </p>
                    </div>
                    <button
                ...
                {placementKeys.map((pk) => {
                    ...
                    return (
                        <section key={pk} className="mb-7">
                            ...
                            {list.length === 0 ? (
                                <div className="rounded-[12px] border border-dashed border-cardBorder py-8 text-center text-[13px] text-moss">
                                ...
                                >
                                    {list.map((b) => (
                                        <div
                                            key={b.id}
                                            className="overflow-hidden rounded-[14px] border border-cardBorder bg-white"
                                        >
                                            <div
                                                className="relative"
                                                style={{
                                            ...
                                            >
                                                ...
                                                />
                                                {!b.is_active && (
                                                    <span className="absolute left-2 top-2 rounded-full bg-black/55 px-2.5 py-1 text-[11px] font-semibold text-white">
                                                        Đang ẩn
                                                    </span>
                                                )}
                                                {b.link_url && (
                                                    <span className="absolute right-2 top-2 rounded-full bg-grass/90 px-2 py-1 text-[10.5px] font-semibold text-white">
                                                        link
                                                    </span>
                                                )}
                                            </div>
                                            <div className="p-3.5">
                                                <div className="truncate text-[14px] font-bold text-pine">
                                                    {b.title || (
                                                ...
                                                {b.subtitle && (
                                                    <div className="truncate text-[12px] text-moss">
                                                        {b.subtitle}
                                                    </div>
                                                )}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            >
                                {modalMode === 'edit' && editing && (
                                    <img
                                        src={editing.image}
                                        alt=""
                                        className="mb-2 w-full rounded-[10px] border border-cardBorder object-cover"
                            ...
                            <Field label="Mô tả phụ">
                                <input
                                    ...
                                    className={inputCls}
                                    placeholder="Dòng nhỏ dưới tiêu đề (tuỳ chọn)"
                                />
                            </Field>
                            <Field
                            ...
                            >
                                <input
                                    ...
                                    className={inputCls}
                                    placeholder="/thiet-bi hoặc https://..."
                                />
                            </Field>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                    <div className="w-full max-w-sm rounded-[18px] bg-white p-6 shadow-xl">
                        ...
                        <p className="mb-4 text-[13px] text-moss">
                            Ảnh banner cũng sẽ bị xoá. Hành động không thể hoàn
                            tác.
                        </p>
