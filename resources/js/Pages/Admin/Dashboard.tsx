        ...
        <>
            ...
            <div className="mx-auto max-w-[980px]">
                ...
                <p className="mb-5 text-[14px] text-moss">
                    Tình hình đơn thuê và doanh thu của shop.
                </p>

                {/* Doanh thu */}
                <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    <BigStat
                        label="Doanh thu (đơn đã trả)"
                ...
                <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-5">
                    {STATUS_ORDER.map((s) => (
                        <Link
                            ...
                            className="rounded-[12px] border border-cardBorder bg-card p-3 transition hover:border-grass"
                        >
                            <div
                                className="font-mono text-[20px] font-extrabold"
                                style={{ color: STATUS_STYLE[s].color }}
                            >
                                {stats[s]}
                            </div>
                            <div className="text-[12px] text-moss">
                                {STATUS_LABEL[s]}
                            </div>
                        </Link>
                    ))}
                    ...
                    <div className="mb-6 rounded-card border border-cardBorder bg-card p-5">
                        <div className="mb-3 flex items-center justify-between">
                            <h2 className="text-[16px] font-bold text-ink">
                                Combo — lượt thuê &amp; convert
                            </h2>
                            <Link
                                href="/admin/combos"
                                className="text-[13px] font-semibold text-grass hover:text-pine"
                            >
                                Quản lý combo →
                            </Link>
                        </div>
                            ...
                            <table className="w-full text-[14px]">
                                ...
                                <tbody>
                                    {combo_stats.map((c) => (
                                        <tr
                                            key={c.id}
                                            className="border-b border-cardBorder/60 last:border-0"
                                        >
                                            <td className="py-2.5 pr-3 font-semibold text-ink">
                                                {c.name}
                                            </td>
                                            <td className="py-2.5 pr-3 text-right font-mono font-bold text-pine">
                                                {c.rentals}
                                            </td>
                                            <td className="py-2.5 pr-3 text-right font-mono text-moss">
                                                {c.shown}
                                            </td>
                                            <td className="py-2.5 pr-3 text-right font-mono text-moss">
                                                {c.converted}
                                            ...
                                            <td className="py-2.5 text-right">
                                                {c.convert_rate === null ? (
                                                    <span className="text-[12px] text-[#a3ad92]">
                                                        chưa có dữ liệu
                                                    </span>
                                                ) : (
                                                    <span
                                        ...
                                        >
                                            <td className="py-2.5 pr-3">
                                                <Link
                                                    ...
                                                    className="font-mono font-bold text-grass hover:text-pine"
                                                >
                                                    {o.code}
                                                </Link>
                                            </td>
...
function BigStat({
    ...
    return (
        <div
            className={`rounded-card border p-4 ${accent ? 'border-grass/40 bg-[#eef2e3]' : 'border-cardBorder bg-card'}`}
        >
            <div
                className={`font-mono text-[22px] font-extrabold ${accent ? 'text-pine' : 'text-ink'}`}
            >
                {value}
            </div>
