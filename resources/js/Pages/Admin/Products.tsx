            ...
            onError: (errors) =>
                setDeleteError(
                    (errors as Record<string, string>).message ??
                        'Xoá không thành công, thử lại nhé.',
                ),
        });
    };
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-0.5 text-[13px] text-moss">
                            <span className="font-mono">{products.total}</span>{' '}
                            sản phẩm
                        </p>
                    </div>
                    <button
                ...
                <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
                    <div className="relative flex-1">
                        <svg
                            ...
                            viewBox="0 0 24 24"
                            fill="none"
                            className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#8a967a]"
                        >
                            <circle
                                cx="11"
                                cy="11"
                    ...
                    {products.data.length === 0 ? (
                        ...
                        <table className="w-full text-[13px]">
                            <thead>
                                ...
                                >
                                    <th className="w-12 px-4 py-3 text-left font-semibold text-moss">
                                        Ảnh
                                    </th>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                    <div className="w-full max-w-sm rounded-[18px] bg-white p-6 shadow-xl">
                        ...
                        <p className="mb-4 text-[13px] text-moss">
                            Tất cả ảnh của sản phẩm cũng sẽ bị xoá. Hành động
                            không thể hoàn tác.
                        </p>
                        {/* US-07: xoá sản phẩm thuộc combo → combo tự ẩn */}
                        {(() => {
                            const comboNames =
                                products.data.find((p) => p.id === deleteId)
                                    ?.combo_names ?? [];
                            return comboNames.length > 0 ? (
                                <p className="mb-4 rounded-[9px] bg-[#fdf3f1] px-3.5 py-2.5 text-[12.5px] font-semibold text-[#b3493a]">
                                    ...
                                    {comboNames.join(', ')}. Xoá sản phẩm sẽ tự
