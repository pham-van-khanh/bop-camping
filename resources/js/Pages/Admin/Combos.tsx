...
type ComboImage = { id: number; path: string; sort_order: number; type: 'image' | 'video' };
type ProductOption = {
    id: number;
    name: string;
...
export default function AdminCombos({ combos, products }: { combos: Combo[]; products: ProductOption[] }) {
    ...
    const sumIndividual = form.data.items.reduce(
        (sum, item) => sum + (productById.get(item.product_id)?.price_per_day ?? 0) * item.quantity,
        0,
    );
    const comboPrice = form.data.combo_price === '' ? 0 : form.data.combo_price;
    const savings = sumIndividual - comboPrice;
    const savingsPercent = sumIndividual > 0 ? Math.round((savings * 100) / sumIndividual) : 0;
    const overPriced = form.data.items.length > 0 && form.data.combo_price !== '' && comboPrice >= sumIndividual;
    ...
    const removeItem = (productId: number) => {
        form.setData('items', form.data.items.filter((item) => item.product_id !== productId));
    };
    ...
    const openEdit = (c: Combo) => {
        form.setData({
            ...
            items: c.items.map((item) => ({ product_id: item.product_id, quantity: item.quantity })),
            confirm_over_price: false,
        });
        form.clearErrors();
        setProductSearch('');
        ...
        <>
            ...
            <div className="p-6">
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h1 className="text-[22px] font-extrabold text-pine">Combo</h1>
                        <p className="mt-0.5 text-[13px] text-moss">
                            <span className="font-mono">{combos.length}</span> combo · click vào hàng để quản lý ảnh
                        </p>
                    </div>
                    <button
                        onClick={openCreate}
                        ...
                        <table className="w-full text-[13px]">
                            <thead>
                                <tr className="border-b border-[#eef2e3]" style={{ background: '#f8faf4' }}>
                                    <th className="px-4 py-3 text-left font-semibold text-moss">Combo</th>
                                    <th className="hidden px-4 py-3 text-center font-semibold text-moss sm:table-cell">Số món</th>
                                    <th className="px-4 py-3 text-right font-semibold text-moss">Giá combo/ngày</th>
                                    <th className="hidden px-4 py-3 text-right font-semibold text-moss md:table-cell">Tổng giá lẻ</th>
                                    <th className="px-4 py-3 text-center font-semibold text-moss">Tiết kiệm</th>
                                            ...
                                            >
                                                <td className="px-4 py-3">
                                                    ...
                                                    <div className="font-mono text-[11px] text-moss">
                                                        {c.slug}
                                                        {c.suitable_for ? ` · ${c.suitable_for} người` : ''}
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-4 py-3 text-right">
                                                    <div className="font-mono font-bold text-pine">{money(c.combo_price)}</div>
                                                    {c.deposit ? (
                                                        <div className="font-mono text-[11px] text-campfire">cọc {money(c.deposit)}</div>
                                                    ) : null}
                                                </td>
                                                ...
                                                <td className="px-4 py-3 text-center">
                                                    <span
                                                        className={`rounded-pill px-2.5 py-1 text-[11.5px] font-bold ${
                                                            c.is_active ? 'bg-[#eef5e1] text-grass' : 'bg-[#f1f4ea] text-moss'
                                                        }`}
                                                    >
                                                        {c.is_active ? 'Đang bán' : 'Ẩn'}
                                                    </span>
                                                </td>
                                                <td className="px-4 py-3 text-right">
                                                    ...
                                                    <td colSpan={7} className="px-6 pb-5 pt-3" style={{ background: '#fafcf7' }}>
                                                        ...
                                                        <div className="mb-4">
                                                            <div className="mb-2 text-[12.5px] font-semibold text-moss">
                                                                Món trong combo ({c.items.length})
                                                            </div>
            ...
            {modalMode && (
                <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 px-4 py-8" onClick={closeModal}>
                    <div className="w-full max-w-xl rounded-[18px] bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
                        <h2 className="mb-5 text-[18px] font-extrabold text-pine">
                            {modalMode === 'create' ? 'Thêm combo' : 'Sửa combo'}
                        </h2>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="mb-1.5 block text-[13px] font-semibold text-pine">
                                    Tên combo <span className="text-[#b3493a]">*</span>
                                </label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="mb-1.5 block text-[13px] font-semibold text-pine">
                                    Sản phẩm trong combo <span className="text-[#b3493a]">*</span>
                                    <span className="ml-1 font-normal text-moss">(khuyến nghị ≤ 8 món)</span>
                                </label>
                                ...
                                {form.data.items.length > 0 && (
                                    <div className="mb-2 space-y-1.5">
                                        {form.data.items.map((item) => {
                                            ...
                                            return (
                                                <div key={item.product_id} className="flex items-center gap-2 rounded-[10px] border border-cardBorder bg-[#fafcf7] px-3 py-2">
                                                    <span className="flex-1 text-[13px] font-semibold text-pine">
                                                        {p?.name ?? `#${item.product_id}`}
                                                        {p?.status === 'hidden' && (
                                                            <span className="ml-1.5 rounded-pill bg-[#f6ddd6] px-1.5 py-0.5 text-[10px] font-bold text-[#b3493a]">đang ẩn</span>
                                                        )}
                                                        <span className="ml-1.5 font-mono text-[11px] font-normal text-moss">
                                                            {money(p?.price_per_day ?? 0)}/ngày · kho {p?.quantity ?? '?'}
                                                        </span>
                                                    </span>
                                                    <input
                                                        type="number"
                                                        ...
                                                        max={100}
                                                        value={item.quantity}
                                                        onChange={(e) => setItemQuantity(item.product_id, Math.max(1, Number(e.target.value)))}
                                ...
                                {productSearch && (
                                    <div className="mt-1 max-h-44 overflow-y-auto rounded-[10px] border border-cardBorder bg-white shadow-sm">
                                        {filteredProducts.length === 0 ? (
                                            <div className="px-3.5 py-2.5 text-[12.5px] text-moss">Không tìm thấy sản phẩm phù hợp</div>
                                        ) : (
                                            filteredProducts.slice(0, 8).map((p) => (
                                                ...
                                                >
                                                    ...
                                                    <span className="font-mono text-[11.5px] text-moss">{money(p.price_per_day)}/ngày</span>
                                                </button>
                                            ))
                                        )}
                                    </div>
                                )}
                                {form.errors.items && <p className="mt-1 text-[12px] text-[#b3493a]">{form.errors.items}</p>}
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <div>
                                    <label className="mb-1.5 block text-[13px] font-semibold text-pine">
                                        Giá combo/ngày (₫) <span className="text-[#b3493a]">*</span>
                                    </label>
                                    <input
                                        type="number"
                                        min="0"
                                        value={form.data.combo_price}
                                        onChange={(e) => form.setData('combo_price', e.target.value === '' ? '' : Number(e.target.value))}
                                        ...
                                        placeholder="150000"
                                    />
                                    {form.errors.combo_price && <p className="mt-1 text-[12px] text-[#b3493a]">{form.errors.combo_price}</p>}
                                </div>
                                <div>
                                ...
                                <div>
                                    <label className="mb-1.5 block text-[13px] font-semibold text-pine">Số người phù hợp</label>
                                    <input
                                        type="number"
                                        min="1"
                                        max="100"
                                        value={form.data.suitable_for ?? ''}
                                        onChange={(e) => form.setData('suitable_for', e.target.value === '' ? null : Number(e.target.value))}
                                        className="w-full rounded-[10px] border border-cardBorder px-3 py-2.5 text-[13.5px] outline-none transition focus:border-grass"
                                        placeholder="4"
                                    />
                                </div>
                            ...
                            {form.data.items.length > 0 && (
                                <div
                                    className={`rounded-[12px] border px-4 py-3 text-[13px] ${
                                        overPriced ? 'border-[#f0c9c0] bg-[#fdf3f1]' : 'border-[#dcebc4] bg-[#f6faee]'
                                    }`}
                                >
                                    <div className="flex items-center justify-between">
                                        <span className="text-moss">Tổng giá thuê lẻ</span>
                                        <span className="font-mono font-bold text-pine">{money(sumIndividual)}/ngày</span>
                                    </div>
                                    {form.data.combo_price !== '' && (
                                        <div className="mt-1 flex items-center justify-between">
                                            <span className="text-moss">Khách tiết kiệm</span>
                                            {overPriced ? (
                                                <span className="font-mono font-bold text-[#b3493a]">không tiết kiệm</span>
                                            ) : (
                                                <span className="font-mono font-bold text-grass">
                                                    {money(savings)}/ngày (−{savingsPercent}%)
                                                </span>
                                            )}
                                        </div>
                                    )}
                                    {overPriced && (
                                        <label className="mt-2 flex cursor-pointer items-center gap-2 text-[12.5px] font-semibold text-[#b3493a]">
                                            <input
                                                type="checkbox"
                                                checked={form.data.confirm_over_price}
                                                onChange={(e) => form.setData('confirm_over_price', e.target.checked)}
                                                className="accent-[#b3493a]"
                                            />
                                            Giá combo không rẻ hơn thuê lẻ — vẫn lưu (chủ đích)
                                        </label>
                                    )}
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                        ...
                                        {([true, false] as const).map((v) => (
                                            <label key={String(v)} className="flex cursor-pointer items-center gap-2">
                                                <input
                                                    type="radio"
                                                    name="is_active"
                                                    checked={form.data.is_active === v}
                                                    onChange={() => form.setData('is_active', v)}
                                                    className="accent-grass"
                                                />
                                                <span className="text-[13px] text-pine">{v ? 'Đang bán' : 'Ẩn'}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                                <div>
                                    <label className="mb-1.5 block text-[13px] font-semibold text-pine">
                                        Thứ tự hiển thị <span className="font-normal text-moss">(nhỏ = lên trước)</span>
                                    </label>
                                    <input
                                        type="number"
                                        min="0"
                                        value={form.data.sort_order}
                                        onChange={(e) => form.setData('sort_order', Math.max(0, Number(e.target.value)))}
                                        className="w-full rounded-[10px] border border-cardBorder px-3 py-2.5 text-[13.5px] outline-none transition focus:border-grass"
                                    />
                                </div>
                            </div>
            ...
            {deleteId !== null && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                    <div className="w-full max-w-sm rounded-[18px] bg-white p-6 shadow-xl">
                        <h2 className="mb-2 text-[16px] font-extrabold text-pine">Xác nhận xoá combo</h2>
                        <p className="mb-4 text-[13px] text-moss">
                            Ảnh của combo cũng sẽ bị xoá; đơn hàng cũ không bị ảnh hưởng. Hành động không thể hoàn tác.
                        </p>
