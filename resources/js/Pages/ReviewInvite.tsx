...
type ItemForm = { order_item_id: number; rating: number; content: string; media: File[] };
...
export default function ReviewInvite() {
    ...
    const { data, setData, post, processing, errors, transform } = useForm<{
        ...
        system_rating: 0,
        system_content: '',
        items: (items ?? []).map((i) => ({ order_item_id: i.order_item_id, rating: 0, content: '', media: [] })),
    });
    ...
    const submit = () => {
        ...
        post(route('review.invite.store', token), { forceFormData: true, preserveScroll: true, onSuccess: () => setDone(true) });
    };
            ...
            <main className="mx-auto max-w-[640px] px-5 pb-16 pt-8">
                {!found ? (
                    ...
                    <>
                        ...
                        {mediaError && (
                            <p className="mt-3 text-[13px] text-red-500">Có tệp không hợp lệ (chỉ nhận ảnh/video, mỗi tệp ≤30MB, tối đa 4/sản phẩm).</p>
                        )}
                        ...
                        <p className="mt-2 text-center font-mono text-[12px] text-moss">Chỉ cần chấm sao mục bạn muốn đánh giá. Thêm ảnh/video để đánh giá sinh động hơn.</p>
                    </>
                )}
            </main>
        </>
...
function ItemCard({ name, value, onChange }: { name: string; value?: ItemForm; onChange: (patch: Partial<ItemForm>) => void }) {
    const fileRef = useRef<HTMLInputElement>(null);
    const media = value?.media ?? [];

    const previews = useMemo(
        () => media.map((f) => ({ url: URL.createObjectURL(f), type: f.type.startsWith('video/') ? 'video' as const : 'image' as const })),
        [media],
    );
    ...
    const removeAt = (i: number) => onChange({ media: media.filter((_, j) => j !== i) });

    return (
        <Card className="mt-3">
            <div className="mb-3 flex items-center justify-between gap-3">
                <h2 className="text-[15px] font-bold text-ink">{name}</h2>
                <Stars value={value?.rating ?? 0} onPick={(n) => onChange({ rating: n })} />
            </div>
            <textarea value={value?.content ?? ''} onChange={(e) => onChange({ content: e.target.value })}
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-2.5">
                {previews.map((p, i) => (
                    <div key={i} className="relative" style={{ width: 56, height: 56 }}>
                        ...
                        <button type="button" onClick={() => removeAt(i)} aria-label="Xoá"
                            className="absolute -right-1.5 -top-1.5 grid h-5 w-5 place-items-center rounded-full bg-pine text-[12px] text-white">×</button>
                    </div>
                ))}
                {media.length < 4 && (
                    <button type="button" onClick={() => fileRef.current?.click()}
                        className="grid place-items-center rounded-[11px] text-grass" style={{ width: 56, height: 56, border: '1.5px dashed #b9c79a' }}>
                        <span className="text-[18px] leading-none">＋</span>
                        <span className="font-mono text-[10px]">Ảnh/video</span>
                    </button>
                )}
                <span className="font-mono text-[12px] text-moss">{media.length}/4</span>
