...
import {
    cartCount, cartTotals, clearCart, getCart, lineDays, lineDeposit, lineRent, removeLine, setQty, type CartLine,
} from '@/lib/cart';
import { money, rangeText } from '@/lib/format';
import { getUser } from '@/lib/auth';
import { on, EVENTS } from '@/lib/bus';

type Placed = { code: string; name: string; itemsText: string; payText: string };
...
export default function Cart() {
    ...
    useEffect(() => {
        ...
        return on(EVENTS.cartChange, () => setLines(getCart()));
    }, []);

    const totals = cartTotals(lines);
    const valid = form.name.trim().length >= 2 && form.phone.trim().length >= 8 && form.address.trim().length >= 4;
    ...
    const placeOrder = () => {
        ...
        const code = `BOP-${1000 + Math.floor(Math.random() * 9000)}`;
        const snapshot: Placed = {
            code,
            name: form.name.trim(),
            itemsText: `${lines.length} món · ${cartCount(lines)} bộ`,
            payText: money(totals.pay),
        };
        clearCart();
        setPlaced(snapshot);
                ...
                <main className="mx-auto max-w-[1060px] px-5 pb-12 pt-[30px]">
                    <div className="mx-auto mt-2.5 max-w-[600px] rounded-[20px] border border-cardBorder bg-card text-center" style={{ padding: 'clamp(28px,4vw,44px)' }}>
                        ...
                        <div className="mb-[22px] rounded-[13px] px-[18px] py-4 text-left" style={{ background: '#f6f8ef' }}>
                            <Row k="Khách" v={placed.name} />
                            <Row k="Số món" v={placed.itemsText} mono />
                            <Row k="Trả khi nhận (COD)" v={placed.payText} mono accent />
                        </div>
            ...
            <main className="mx-auto max-w-[1060px] px-5 pb-12 pt-[30px]">
                ...
                {lines.length === 0 ? (
                    <div className="rounded-[18px] border border-dashed px-6 py-14 text-center" style={{ borderColor: '#cdd6b6', background: '#FBFCF7' }}>
                        <div className="mb-3 text-[40px]" style={{ animation: 'skeleton 3s ease-in-out infinite' }}>🎒</div>
                    ...
                    <div className="grid items-start gap-6" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
                        ...
                        <div className="sticky top-[84px] rounded-card border border-cardBorder bg-card p-5">
                            ...
                            <div className="mb-4 flex flex-col gap-[11px]">
                                <input value={form.name} onChange={set('name')} placeholder="Họ và tên" className="h-[46px] rounded-[11px] border border-cardBorder bg-white px-3.5 text-[15px] text-ink outline-none focus:border-grass" />
                                <input value={form.phone} onChange={set('phone')} placeholder="Số điện thoại" inputMode="tel" className="h-[46px] rounded-[11px] border border-cardBorder bg-white px-3.5 text-[15px] text-ink outline-none focus:border-grass" />
                                <input value={form.address} onChange={set('address')} placeholder="Địa chỉ giao nhận" className="h-[46px] rounded-[11px] border border-cardBorder bg-white px-3.5 text-[15px] text-ink outline-none focus:border-grass" />
                                <textarea value={form.note} onChange={set('note')} placeholder="Ghi chú (tuỳ chọn)" rows={2} className="resize-y rounded-[11px] border border-cardBorder bg-white px-3.5 py-[11px] text-[14px] text-ink outline-none focus:border-grass" />
                            </div>
                            ...
                            {!valid && <div className="mt-2 text-center text-[12px] text-[#8a967a]">Điền tên, số điện thoại và địa chỉ để đặt đơn.</div>}
                        </div>
                    </div>
                )}
            </main>
...
function Row({ k, v, mono, accent, accentWarm }: { k: string; v: string; mono?: boolean; accent?: boolean; accentWarm?: boolean }) {
    return (
        <div className="flex justify-between py-[5px] text-[14px]">
            ...
            <span className={`${mono ? 'font-mono' : ''} ${accent ? 'font-bold' : 'font-semibold'}`} style={{ color: accent ? '#557A2B' : accentWarm ? '#C97B36' : '#18230F' }}>{v}</span>
        </div>
    );
}
