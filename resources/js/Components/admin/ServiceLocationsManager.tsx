...
const STATUS_LABEL: Record<ServiceLocation['status'], string> = {
    open: 'Đang mở',
    coming: 'Sắp mở',
};
const inputCls =
...
export default function ServiceLocationsManager({
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (modalMode === 'create')
            form.post(route('admin.service-locations.store'), opts);
        else if (editing)
            form.put(route('admin.service-locations.update', editing.id), opts);
    };
    ...
    const deleting =
        deleteId !== null ? locations.find((l) => l.id === deleteId) : null;
    ...
    return (
        <div className="mb-6 rounded-[16px] border border-cardBorder bg-white p-4">
            <div className="mb-3 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-[12px] text-moss">
                        Nơi shop giao nhận — hiện ở panel "Đang phục vụ tại"
                        trang chủ
                    </p>
                </div>
                <button
                    onClick={openCreate}
                    className="flex items-center gap-1.5 rounded-[10px] border border-cardBorder bg-white px-3 py-1.5 text-[12.5px] font-semibold text-pine transition hover:border-grass hover:text-grass"
                >
            ...
            {locations.length === 0 ? (
                <div className="rounded-[10px] border border-dashed border-cardBorder py-5 text-center text-[12.5px] text-moss">
                    Chưa có vị trí phục vụ nào
                </div>
            ) : (
                <div className="flex flex-wrap gap-2.5">
                    {locations.map((l) => (
                        <div
                            key={l.id}
                            className="group flex items-center gap-2.5 rounded-[12px] border border-cardBorder bg-[#fafcf7] px-3 py-2"
                        >
                            <svg
                                width="15"
                                height="15"
                            ...
                            <div className="leading-tight">
                                <div className="text-[13px] font-semibold text-pine">
                                    {l.name}
                                </div>
                                <div className="text-[11px] text-moss">
                                    {l.area ?? '—'} · {l.spots_count} điểm
                                </div>
                            </div>
                            <span
                            ...
                            <div className="ml-1 flex items-center gap-1 opacity-0 transition group-hover:opacity-100">
                                <button
                                    onClick={() => openEdit(l)}
                                    title="Sửa"
                                    className="grid h-6 w-6 place-items-center rounded-[7px] border border-cardBorder text-pine hover:border-grass hover:text-grass"
                                >
                                    <svg
                                        width="12"
                                        height="12"
                                    ...
                                    >
                                        <path
                                            d="M4 20h4L18.5 9.5a2 2 0 0 0-3-3L5 17v3Z"
                                            stroke="currentColor"
                                            strokeWidth="1.7"
                                ...
                                <button
                                    ...
                                    title="Xoá"
                                    className="grid h-6 w-6 place-items-center rounded-[7px] border border-[#f6ddd6] text-[#b3493a] hover:bg-[#f6ddd6]"
                                >
                                    <svg
                                        width="12"
                                        height="12"
                                    ...
                                    >
                                        <path
                                            d="M5 7h14M10 11v6M14 11v6M6 7l1 12h10l1-12M9 7V4h6v3"
                                            stroke="currentColor"
                                            strokeWidth="1.6"
                                            strokeLinecap="round"
                                            strokeLinejoin="round"
