...
export default function PickupReturnNote({
    ...
    const site = (
        usePage().props as {
            site?: { pickup_hour?: number; return_hour?: number };
        }
    ).site;
    ...
    return (
        <p
            className={`flex items-start gap-1.5 text-[12px] text-moss ${className}`}
        >
            <span aria-hidden>🕗</span>
            <span>
                Nhận từ{' '}
                <span className="font-semibold text-ink">{pickup}h</span>, trả
                trước <span className="font-semibold text-ink">{ret}h</span>.
                Ngoài khung giờ vui lòng liên hệ để sắp xếp.
            </span>
        </p>
    );
}
