...
export default function AdminLayout({ children }: { children: ReactNode }) {
    ...
    return (
        <div className="flex min-h-screen" style={{ background: '#f4f6ef' }}>
            {/* Sidebar */}
            <aside className="flex w-[220px] flex-none flex-col border-r border-[#dde4cc]" style={{ background: '#fff' }}>
                {/* Logo */}
                <div className="flex h-[60px] items-center gap-2.5 border-b border-[#dde4cc] px-5">
                ...
                <nav className="flex-1 px-3 py-4">
                    {NAV.map((item) => {
                        ...
                        if (item.disabled) {
                            return (
                                <div key={item.href} className="mb-1 flex cursor-not-allowed items-center gap-2.5 rounded-[10px] px-3 py-2.5 text-[13.5px] font-semibold text-[#bcc6a0]">
                                    <span className="opacity-40">{item.icon}</span>
                                    {item.label}
                                    <span className="ml-auto rounded-[5px] bg-[#f1f4ea] px-1.5 py-0.5 text-[10px] font-mono text-[#b0ba98]">soon</span>
                                </div>
                            );
                        }
                        return (
                ...
                {/* User + logout */}
                <div className="border-t border-[#dde4cc] p-3">
